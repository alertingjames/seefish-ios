...
class PickLocationViewController: BaseViewController, CLLocationManagerDelegate, GMSMapViewDelegate {
    ...
    var mapHintDialog:MapHintDialog!

    override func viewDidLoad() {
        super.viewDidLoad()
        
        mapHintDialog = UIStoryboard(name: "Main2", bundle: nil).instantiateViewController(identifier: "MapHintDialog")
        mapHintDialog.view.frame = CGRect(x: 0, y: 0, width: screenWidth, height: screenHeight)
    ...
    func showHint() {
        DispatchQueue.main.asyncAfter(deadline: .now() + 2.5) {
            UIView.animate(withDuration: 0.3) {
                self.addChild(self.mapHintDialog)
                self.view.addSubview(self.mapHintDialog.view)
            }
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.3) {
                self.mapHintDialog.showDialog()
            }
        }
    }
